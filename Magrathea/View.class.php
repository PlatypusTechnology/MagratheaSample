...
class MagratheaView{
	...
	*/
	...
	public function ShouldCompressJavascript(){
		return MagratheaConfigStatic::GetConfig($GLOBALS['environment']."/compress_js");
	}
	public function ShouldCompressCss(){
		return MagratheaConfigStatic::GetConfig($GLOBALS['environment']."/compress_css");
	}
	
	public function IncludeJavascript($js_file){
		array_push($this->javascript_files, $js_file);
		$file_full = $GLOBALS['site_path'].$js_file;
	...
	public function IncludeCSS($css_file){
		...
		$file_full = $GLOBALS['site_path'].$css_file;
		if(file_exists($file_full)){
			$lastm = filemtime($file_full);
		} else {
			throw new MagratheaViewException("File ".$file_full." could not be reached!");
	...
	public function Javascripts($compression=null){
		...
		if($compression == "true"){
			...
			$jsContent .= "<script type='text/javascript' src='".($this->relativePath ? "" : "/").$compressedFileName."'></script>\n"; 
  		} else {
			foreach($array_files as $file){
				$jsContent .= "<script type='text/javascript' src='".($this->relativePath ? "" : "/").$file."'></script>\n"; 
			}
		}
		return $jsContent;
	}
	...
	public function CSSs($compression=null){
		...
		if($compression == "true"){
			...
			$cssContent .= "<link href='".($this->relativePath ? "" : "/").$compressedFileName."' rel='stylesheet'>\n"; 
  		} else {
			foreach($array_files as $file){
				$cssContent .= "<link href='".($this->relativePath ? "" : "/").$file."' rel='stylesheet'>\n"; 
			}
		}
		return $cssContent;
	}
