...
foreach( $relations as $rel ){
	...
	$relationsProperties .= "<li>".$rel["rel_property"]." (".$returns.")&nbsp;<i class='icon-exchange' title='relational property'></i></li>";
	$relationsMethods .= "<li>".$obj_name."-&gt;".$rel["rel_method"]."();&nbsp;<i class='icon-exchange' title='relational method'></i><br/>Gets object related ".$returns."</li>";
	if($rel["rel_type"] == "belongs_to"){
		$relationsMethods .= "<li>".$obj_name."-&gt;Set".$rel["rel_object"]."();&nbsp;<i class='icon-exchange' title='relational method'></i><br/>Sets a object as ".$rel["rel_object"]."</li>";
	}
	if($rel["rel_type"] == "has_and_belongs_to_many"){
		$relationsMethods .= "<li>".$obj_name."-&gt;Add".$rel["rel_object"]."();&nbsp;<i class='icon-exchange' title='relational method'></i><br/>Adds a new ".$rel["rel_object"]."</li>";
	}
}
...
<input type="hidden" id="object_name" name="object_name" value="<?=$obj_name?>"/>
<div class="row-fluid">
	<div class="span12 mag_section">
...
<?
	...
	foreach($obj_data as $key => $item){
		if( substr($key, -6) == "_alias" ){
			...
			echo "<li>".(empty($obj_data[$field_name."_alias"]) ? $field_name : $item)." (".$obj_data[$field_name."_type"].") ".($field_name == $obj_data["db_pk"] ? "<i class='icon-key' title='primary key'></i>" : "" )."</li>";
		}
	}
...
<div class="row-fluid">
	<div class="span12 mag_section">
		...
		<content>
			<div class='row-fluid'>
				<div class='span3'><i class='icon-table'></i>&nbsp;&nbsp;&nbsp;Table:</div>
				<div class='span9'>
					<?=$obj_data["table_name"]?>
				</div>
			</div>
			...
			<div class='row-fluid'>
				<div class='span9'>&nbsp;</div>
				<div class='span3'>
					<button class="btn btn-success" onClick="saveObject();"><i class="icon-ok-circle"></i>&nbsp;Save Object</button>
				</div>
			</div>
		</content>
	</div>
...
<div class="row-fluid">
	<div class="span12 mag_section">
		...
		<content>
			<div class="row-fluid" id="add_new_relation_button">
				<button class="btn" onClick="ShowHideNewRelation();"><i class="icon-plus"></i>&nbsp;Create new relation</button>
			</div>
			<div class="row-fluid" id="add_new_relation" style="display: none;">
				<ul class="nav nav-tabs">
					<li class="active"><a><i class="icon-plus"></i>&nbsp;Create new relation</a></li>
				</ul>
	...
	?>
					...
					<div class="row-fluid">
						...
						<div class="span4">
							<button class="btn btn-success" onClick="addNewRelation(); return false;"><i class="icon-ok"></i>&nbsp;Add Relation</button>
							<button class="btn btn-danger" onClick="ShowHideNewRelation(); return false;"><i class="icon-remove"></i>&nbsp;Cancel</button>
						</div>
					</div>
				</div>
			</div>
...
function addNewRelation(){
	...
	var rel_field = "";
	if( rel_type == "has_and_belongs_to_many" ){
		rel_field = $("#relation_table option:selected").val() + "#" +
					$("#object_name").val() + ":" +  $("#relation_table_id_for_obj option:selected").val() + "#" +
					rel_object + ":" + $("#relation_table_id_for_rel option:selected").val();
	} else {
		rel_field = $("#relation_field option:selected").val();
	}
...
function HtmlRelation(relation_name, rel_type, rel_type_val, rel_object, rel_field, method, property){
	...
	optionsButtons += '<button class="btn btn-danger" onClick="DeleteRelation(\''+relation_name+'\'); return false;"><i class="icon-trash"></i>&nbsp;Delete</button>&nbsp;';
	optionsButtons += '</div>';
	var code_manag = '<div class="row-fluid" id="'+relation_name+'_code">';
	code_manag += '<div class="span5">Public Property:<div class="input-append"><input type="text" name="'+relation_name+'_property" id="'+relation_name+'_property" value="'+property+'"><button class="btn btn-danger" onClick="$('+relation_name+'_property).val(\''+property+'\');" type="button"><i class="icon-off"></i> (reset)</button></div></div>';
	code_manag += '<div class="span5">Public Method:<div class="input-append"><input type="text" name="'+relation_name+'_method" id="'+relation_name+'_method" value="'+method+'"><button class="btn btn-danger" onClick="$('+relation_name+'_method).val(\''+method+'\');" type="button"><i class="icon-off"></i> (reset)</button></div></div>';
	code_manag += '<div class="span2">&nbsp;</div>';
	code_manag += '</div>';

	var html_content = '<div id="'+relation_name+'">';
	html_content += '<div class="row-fluid">';
	html_content += '<div class="span8">'+rel_type+'&nbsp;<input type="hidden" name="rel_type_text[]" value="'+rel_type+'"><input type="hidden" name="rel_type[]" value="'+rel_type_val+'"><i class="icon-chevron-right"></i>&nbsp;'+rel_object+'<input type="hidden" name="rel_object[]" value="'+rel_object+'">';
	if( rel_type_val == "belongs_to" ){
		html_content += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<i class="icon-table"></i> using '+rel_field+' field]<input type="hidden" name="rel_field[]" value="'+rel_field+'">';
	} else if(rel_type_val == "has_many") {
		html_content += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<i class="icon-table"></i> using '+rel_object+'\'s '+rel_field+' field]<input type="hidden" value="'+rel_field+'" name="rel_field[]">'
	} else if(rel_type_val == "has_and_belongs_to_many") {
		var table = rel_field.split("#");
		html_content += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<i class="icon-table"></i> using '+table[0]+' as relational table]<input type="hidden" value="'+rel_field+'" name="rel_field[]">'
	}
	html_content += '</div>';
...
function DeleteRelation(rel_name){
	...
	$.ajax({
		...
		success: function(data){
			if( data == "<!--false-->" ){
				alert("Some error happened when trying to delete this relation!");	
			} else if (data == "<!--true--->") {
				alert("Relation was deleted");
				$("#"+rel_name).remove();
			}
		}
	});
...
function loadOptionsForManyToMany(obj_name){
	$.ajax({
		url: "?page=object_relations_fields_many_to_many.php",
		type: "POST",
		data: { 
			object: obj_name
		}, 
		success: function(data){
			$("#related_field").html(data);
			$("#related_field").fadeIn("slow");
			$("#relation_table").unbind("change");
			$("#relation_table").bind("change", function(){
				loadRelationalTable($(this).val());
			});
			$("#relation_table_fields").hide();
		}
	});
}

function loadRelationalTable(table_name, obj_name){
	if( table_name == '0' ){
		$("#relation_table_fields").fadeOut("slow");
		return;
	}
	var obj1 = $("#object_name").val();
	var obj2 = $("#relation_object option:selected").val();
	$.ajax({
		url: "?page=object_relations_fields_table.php",
		type: "POST",
		data: { 
			table: table_name,
			object1: obj1,
		...
		success: function(data){
			$("#relation_table_fields").html(data);
			$("#relation_table_fields").fadeIn("slow");
		}
	})
}
...
jQuery( function($) { 
	...
	$("#relation_type").bind("change", function() {
		...
		if( $(this).attr("value") == "belongs_to" ){ 
		...
		} else if($(this).attr("value") == "has_many") {
			loadFieldsFromObject($("#relation_object option:selected").val());
			...
			loadOptionsForManyToMany($("#relation_object option:selected").val());
			$("#relation_object").bind("change", function(){
				var selected = $(this).attr("value");
				loadOptionsForManyToMany(selected);
			});
		}
	});
	prettyPrint();
