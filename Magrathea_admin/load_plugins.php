...
function loadPluginsList($installed=false){
	if($installed)
		$plugins_path = __DIR__.'/../app/plugins/';
	else 
		$plugins_path = __DIR__.'/../Magrathea/plugins/';
				...
				<div class="singlePlugin">
					<div class='row-fluid <?=($even ? "even" : "")?>' style="padding-top: 5px;">
						...
						<div class="span3 right" style="padding-right: 20px;">
							<button class="btn btn-default" onClick="pluginDetail('<?=$p?>', this);"><i class="icon-eye-open"></i>&nbsp;View details</button>
						</div>
					</div>
					<div class='row-fluid <?=($even ? "even" : "")?> plugin_extras' style="padding-left: 10px; display: none;">
						...
						<div class="span3 plugin_install">
							<button class="btn btn-default" onClick="installPlugin('<?=$p?>');"><i class="icon-download-alt"></i>&nbsp;Install Plugin!</button>
						</div>
					</div>
				</div>
				<?
		...
		<content>
			...
			foreach($plugins as $p){
				...
				<div class='row-fluid <?=($even ? "even" : "")?>' style="padding: 5px;">
					<div class="span9" style="padding-left: 20px;"><?=$p?></div>
					<div class="span3 right" style="padding-right: 20px;">
						<button class="btn btn-default" onClick="removePlugin('<?=$p?>');"><i class="icon-trash"></i>&nbsp;Unninstal</button>
					</div>
				</div>
				<?
				$even = !$even;
